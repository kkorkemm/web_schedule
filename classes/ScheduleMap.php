<?php
    ...
    class ScheduleMap extends BaseMap {
        ...
        public function findSubjectsStudent($studentID, $dayID) {
            $res = $this->db->query(
                "SELECT lesson_num.name as lesson_num, 
                ...
                INNER JOIN lesson_num on schedule.lesson_num_id = lesson_num.name
                INNER JOIN day on schedule.day_id = day.day_id
                INNER JOIN classroom on schedule.classroom_id = classroom.classroom_id
                WHERE user.user_id = $studentID AND day.day_id = $dayID"
            );
            return $res->fetchAll(PDO::FETCH_ASSOC);
        }
